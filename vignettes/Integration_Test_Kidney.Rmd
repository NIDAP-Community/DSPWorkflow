...
```{r Filtering, echo=FALSE}
  ...
  create.rds <- FALSE
  if(create.rds) {
    filtering.human.kidney <- sdesign.list$object
...
```{r Normalization, echo=FALSE}
    ...
    if(create.rds) {
      ...
      saveRDS(normalization.human.kidney, file = "tests/testthat/fixtures/Human_Kidney/normalizationHumanKidney.RDS")
    }
...
```{r Unsupervised Analysis, echo=FALSE}
    ...
    unsupervised.output <- dimReduct(object = normalization.output$object,
                        point.size = 3,
                        point.alpha = 1,
    ...
    create.rds <- FALSE
    if(create.rds) {
      unsupervised.human.kidney <- unsupervised.output$object
      saveRDS(unsupervised.human.kidney, file = "tests/testthat/fixtures/Human_Kidney/unsupervisedAnalysisHumanKidney.RDS")
    }
...
```{r Differential Expression Analysis,  echo=FALSE}
    ...
    object <- normalization.output$object
    object <- object[goi,]
    Gene <- Subset <- NULL
    ...
    #Second analysis:
    reslist.2 <- diffExpr(object = normalization.output$object, 
...
```{r Violin Plot, echo=FALSE}
    ...
    violin.plot.test <- violinPlot(object = normalization.output$object, 
                                        expr.type = "q_norm", 
                                        genes = genes,
                                        group = "region",
                                        facet.by = "segment")
