...
#' @param target.Data NanoStringGeoMxSet object with normalized data
...
UnsupervisedAnalysis <- function(target.Data, point.size = 1, point.alpha = 1) {
  
  # run UMAP
  custom.umap <- umap::umap.defaults
  custom.umap$random_state <- 42
  umap.out <-
    umap(t(log2(assayDataElement(target.Data , elt = "q_norm"))),  
         config = custom_umap)
  pData(target.Data)[, c("UMAP1", "UMAP2")] <- umap.out$layout[, c(1,2)]
  umap.plot <- ggplot(pData(target.Data),
  ...
  tsne.out <-
    Rtsne(t(log2(assayDataElement(target.Data , elt = "q_norm"))),
          perplexity = ncol(target.Data)*.15)
  pData(target.Data)[, c("tSNE1", "tSNE2")] <- tsne.out$Y[, c(1,2)]
  tsne.plot <- ggplot(pData(target.Data),
  ...
  pca.out <-
    prcomp(t(log2(
      assayDataElement(target.Data , elt = "q_norm")
    )), scale. = TRUE)
  pData(target.Data)[, c("PC1", "PC2")] <- pca.out$"x"[, c(1,2)]
  
  pca.plot <- ggplot(pData(target.Data),
  ...
  # return
  invisible(list("dsp.object" = target.Data, "plot" = plot.list))
