...
#'    as a heatmap.
#' 
#' @param object Normalized targetData S4 object input file.
...
#' @param scale.by.row.or.col scale by row or by col ("row", "column" and 
#'        "none")
...
#'        values in mat e.g seq(-3, 3, 0.05), 6/0.05=120 colors.
#' @param heatmap.color colors of heatmap colorRampPalette(c("blue", "white", 
#'        "red"))(120), here 120 is consistent with 120 above.
#' @param norm.method normalization method quant: Upper quartile (Q3) 
#'        normalization and neg: background normalization
#'
...
##
## object is the output S4 obj from normalization step
##
################################################################################
#          pheatmap arguments                                                  #
################################################################################
...
# kmeans_k  the number of kmeans clusters to make, if we want to aggregate the 
...
#           is one element longer than color vector. Used for mapping values to
#           colors. Useful, if needed to map certain values to certain colors
#           colors, to certain values. If value is NA then the breaks are
#           calculated automatically. When breaks don't cover the range of 
#           values, then any value larger than max(breaks) will have the
#           largest color and any value lower than min(breaks) will get the 
#           lowest color.
#
# border_color    color of cell borders on heatmap, use NA if no border should 
#                 be drawn.
...
#                           "euclidean", etc. If the value is none of the 
#                           above it is assumed that a distance matrix is 
#                           provided.
# clustering_distance_cols  distance measure used in clustering columns. 
#                           Possible values the same as for 
#                           clustering_distance_rows.
# clustering_method         clustering method used. Accepts the same values 
#                           as hclust.
# clustering_callback       callback function to modify the clustering. Is 
#                           called with two parameters: original hclust object 
#                           and the matrix used for clustering. Must return a 
#                           hclust object.
# cutree_cols               similar to cutree_rows, but for columns
# treeheight_row            the height of a tree for rows, if these are 
#                           clustered. Default value 50 points.
# treeheight_col            the height of a tree for columns, if these are 
#                           clustered. Default value 50 points.
# legend                    logical to determine if legend should be drawn 
#                           or not.
# legend_breaks             vector of breakpoints for the legend.
# legend_labels             vector of labels for the legend_breaks.
# annotation_row            data frame that specifies the annotations shown 
#                           on left side of the heatmap.
#                           Each row defines the features for a specific 
#                           row. The rows in the data and in
#                           the annotation are matched using corresponding 
#                           row names. Note that color
#                           schemes takes into account if variable is 
#                           continuous or discrete.
# annotation_col            similar to annotation_row, but for columns.
# annotation                deprecated parameter that currently sets the 
#                           annotation_col if it is missing
# annotation_colors         list for specifying annotation_row and 
# annotation_col            track colors manually. It is possible to define 
#                           the colors for only some of the features. Check 
...
#                           annotation tracks should be drawn.
# drop_levels               logical to determine if unused levels are also 
#                           shown in the legend
...
# angle_col                 angle of the column labels, right now one can 
#                           choose only from few predefined options 
#                           (0, 45, 90, 270 and 315)
# display_numbers           logical determining if the numeric values are 
#                           also printed to the cells. If this is a matrix 
#                           (with same dimensions as original matrix), the 
#                           contents of the matrix are shown instead of 
#                           original values.
# number_format             format strings (C printf style) of the numbers 
#                           shown in cells. For example "%.2f" shows 2 decimal 
#                           places and "%.1e" shows exponential notation 
#                           (see more in sprintf).
# number_color              color of the text
# fontsize_number           fontsize of the numbers displayed in cells
# gaps_row                  vector of row indices that show where to put gaps 
#                           into heatmap. Used only if the rows are not 
#                           clustered. See cutree_row to see how to introduce 
#                           gaps to clustered rows.
# gaps_col                  similar to gaps_row, but for columns.
# labels_row                custom labels for rows that are used instead 
#                           of rownames.
# labels_col                similar to labels_row, but for columns.
# filename                  file path where to save the picture. Filetype 
#                           is decided by the extension in the path. 
#                           Currently following formats are supported: png, 
#                           pdf, tiff, bmp, jpeg. Even if the plot does not 
#                           fit into the plotting window, the file size is 
#                           calculated so that the plot would fit there, 
#                           unless specified otherwise.
# width                     manual option for determining the output file 
#                           width in inches.
# height                    manual option for determining the output file 
#                           height in inches.
# silent                    do not draw the plot (useful when using the 
#                           gtable output)
# na_col                    specify the color of the NA cell in the matrix.
# ...                       graphical parameters for the text used in plot. 
#                           Parameters passed to grid.text,see gpar.
